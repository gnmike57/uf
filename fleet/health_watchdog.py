...
class WorkerHeartbeat:
    ...
    def start(self) -> Optional[threading.Thread]:
        ...
        self._thread.start()
        logger.info(
            f"[Heartbeat] Started for worker '{self._worker_id}' "
    ...
    def beat(self) -> bool:
        ...
        try:
            self._redis.hset(
                self._hb_key, self._worker_id, str(int(time.time()))
            )
            return True
        except Exception as e:
...
class FleetWatchdog:
    ...
    def __init__(
        ...
        self._scan_interval = min(self._timeout // 3, 10)  # Scan 3x per timeout window
        self._redis = None
        self._available = False
        self._running = False
        self._stop_event = threading.Event()
    ...
    def start(self) -> None:
        ...
        self._stop_event.clear()
        logger.info(
            f"[Watchdog] Fleet health monitor active. "
            f"Timeout={self._timeout}s, scan every {self._scan_interval}s."
        )
    ...
    def start_background(self) -> threading.Thread:
        ...
        thread = threading.Thread(
            target=self.start, daemon=True, name="fleet-watchdog"
        )
        thread.start()
        return thread
    ...
    def _scan_heartbeats(self) -> None:
        ...
        for worker_id, last_beat_str in workers.items():
            ...
            if silence > self._timeout:
                logger.critical(
                    f"[Watchdog] Worker '{worker_id}' FLATLINED! "
                    f"Last heartbeat {silence}s ago (timeout={self._timeout}s)."
                )
    ...
    def _recover_orphaned_tasks(self, dead_worker_id: str) -> int:
        ...
        try:
            ...
            if not orphans:
                logger.info(
                    f"[Watchdog] No orphaned tasks for worker '{dead_worker_id}'."
                )
                return 0

            logger.warning(
                f"[Watchdog] Found {len(orphans)} orphaned tasks "
                f"from worker '{dead_worker_id}'."
            )

            triaged = 0
            for raw_task in orphans:
                try:
                    # Wrap with triage metadata
                    dlq_entry = json.dumps({
                        "original_task": json.loads(raw_task) if isinstance(raw_task, str) else raw_task,
                        "orphaned_from": dead_worker_id,
                        "triaged_at": time.time(),
                        "reason": "worker_flatlined",
                    })

                    # Atomic: push to DLQ, remove from processing
                    ...
                    logger.error(
                        f"[Watchdog] Orphan triaged to DLQ from worker '{dead_worker_id}'."
                    )
            ...
            logger.error(
                f"[Watchdog] Orphan recovery failed for '{dead_worker_id}': {e}"
            )
            return 0
    ...
    def _fire_alert(self, dead_worker_id: str, orphan_count: int) -> None:
        ...
        try:
            ...
            dlq.capture_failure(
                task_id=f"ORPHAN_{dead_worker_id}",
                error_chain=(
                    f"Worker '{dead_worker_id}' flatlined. "
                    f"{orphan_count} orphaned tasks triaged to DLQ."
                ),
                metadata={
                    "dead_worker_id": dead_worker_id,
                    "orphan_count": orphan_count,
                    "event": "worker_flatlined",
                },
            )
        except Exception as e:
    ...
    def get_fleet_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "alive_workers": sum(1 for w in status.values() if w["alive"]),
                "dead_workers": sum(1 for w in status.values() if not w["alive"]),
            }
