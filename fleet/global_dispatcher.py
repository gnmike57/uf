...
class GlobalDispatcher:
    ...
    def __init__(
        ...
        self._current_raw: Optional[str] = None  # Raw payload of current task
        self._init_redis()
    ...
    def _init_redis(self) -> None:
        ...
        try:
            ...
            self._redis = redis_lib.from_url(
                ...
                socket_timeout=max(self._fetch_timeout + 2, 10),
            )
            self._redis.ping()
            self._available = True
            logger.info(
                f"[Dispatcher] Connected: {self._redis_url} "
                f"(worker={self._worker_id}, queue={self._queue_name})"
            )
        except ImportError:
    ...
    def fetch_next_workflow(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # This is atomic — the task is never "in transit"
            raw_task = self._redis.brpoplpush(
                self._queue_name,
            ...
            logger.info(
                f"[Dispatcher] Worker {self._worker_id} acquired "
    ...
    def mark_workflow_complete(
        ...
        try:
            ...
            if removed:
                logger.info(
                    f"[Dispatcher] Workflow '{workflow_id}' cleared from processing."
                )
            else:
                logger.warning(
                    f"[Dispatcher] Workflow '{workflow_id}' not found in processing queue."
                )
    ...
    def mark_workflow_failed(
        ...
        try:
            # Wrap with failure metadata
            dlq_entry = json.dumps({
                "original_task": json.loads(payload),
                "workflow_id": workflow_id,
                "failed_by": self._worker_id,
                "failed_at": time.time(),
                "error": error,
            })
            ...
            self._current_raw = None
            logger.warning(
                f"[Dispatcher] Workflow '{workflow_id}' moved to DLQ."
            )
            return True
    ...
    def submit_workflow(self, task: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.info(
                f"[Dispatcher] Workflow '{task['workflow_id']}' submitted to queue."
            )
            return True
