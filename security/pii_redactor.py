...
# Core financial PII patterns
_BUILTIN_PATTERNS = [
    # Currency amounts: $10,000.00, $1,450,000.00, USD 5000
    re.compile(
        r"\$\s*\d{1,3}(?:,\d{3})*(?:\.\d{1,2})?", re.IGNORECASE
    ),
    re.compile(
        r"(?:USD|EUR|GBP|AUD)\s*\d{1,3}(?:,\d{3})*(?:\.\d{1,2})?", re.IGNORECASE
    ),

    # Social Security Numbers: 123-45-6789
    re.compile(r"\b\d{3}-\d{2}-\d{4}\b"),

    # Account numbers: Account #123456789, Acc: 987654321, Acct-12345678
    re.compile(
        r"\b(?:Account|Acc|Acct)[\s:#\-]*\d{4,12}\b", re.IGNORECASE
    ),
    ...
    # Routing/ABA numbers: ABA 123456789, Routing: 123456789
    re.compile(
        r"\b(?:ABA|Routing|RTN)[\s:#\-]*\d{9}\b", re.IGNORECASE
    ),

    # Date of birth patterns: DOB: 01/15/1990, Birth Date: 1990-01-15
    re.compile(
        r"\b(?:DOB|Birth\s*Date|Date\s*of\s*Birth)[\s:]*\d{1,2}[/\-]\d{1,2}[/\-]\d{2,4}\b",
        re.IGNORECASE,
    ),

    # Balance amounts: Balance: 15000.00, Available: 12,345.67
    re.compile(
        r"\b(?:Balance|Available|Current|Pending)[\s:]*\$?\s*\d{1,3}(?:,\d{3})*(?:\.\d{1,2})?\b",
        re.IGNORECASE,
    ),
]
...
class PIIRedactor:
    ...
    def redact_uia_tree(
        self, node: Optional[Dict[str, Any]]
    ...
    def redact_screenshot(
        ...
        # Collect bounding boxes of redacted nodes
        blur_regions = []
        if uia_tree:
        ...
        # Apply blur
        try:
            return self._blur_with_pillow(
                screenshot_path, blur_regions, redacted_path
            )
        except ImportError:
            try:
                return self._blur_with_cv2(
                    screenshot_path, blur_regions, redacted_path
                )
            except ImportError:
                logger.warning(
                    "[Redactor] Neither Pillow nor cv2 available for screenshot blur."
                )
                return screenshot_path

    def _collect_redacted_regions(
        self,
        node: Dict[str, Any],
    ...
    def _extract_bbox(
        ...
        """
        # 1. List-based bounding_box: [left, top, right, bottom]
        ...
        # 2. Dict-based adjusted_rectangle (preferred over rectangle)
    ...
    def _blur_with_pillow(
        ...
        for left, top, right, bottom in regions:
            ...
            blurred = roi.filter(
                ImageFilter.GaussianBlur(radius=self._blur_kernel // 2)
            )
            image.paste(blurred, (left, top))

        image.save(dst_path)
        logger.info(
            f"[Redactor] Screenshot PII obfuscated: "
            f"{len(regions)} regions blurred → {dst_path}"
        )
        return dst_path
        ...
        for left, top, right, bottom in regions:
            ...
            if roi.size > 0:
                blurred = cv2.GaussianBlur(
                    roi, (self._blur_kernel, self._blur_kernel), 0
                )
    ...
    def redact_for_cloud(
        ...
        redacted_screenshot = self.redact_screenshot(
            screenshot_path, redacted_tree
        )
