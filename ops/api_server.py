    ...
    if _redis_client is None:
        try:
            ...
            _redis_client = redis.from_url(
                _config.get("REDIS_URL", "redis://127.0.0.1:6379/0"),
...
class HumanResolution(BaseModel):
    ...
    decision: str = Field(
        ..., description="APPROVE, ABORT, or MANUAL_REWIRE"
    )
    override_payload: Optional[str] = Field(
        None, description="Optional replacement instructions for MANUAL_REWIRE"
    )
...
app = FastAPI(
    title="UFO Fleet Control Plane",
    description="Operator monitoring, HITL triage, and fleet management API.",
    version="1.0.0",
)
...
async def get_fleet_status():
    ...
    try:
        ...
        return {
            ...
            "alive": sum(1 for w in worker_list if w["alive"]),
            "dead": sum(1 for w in worker_list if not w["alive"]),
...
async def get_pending_dlq_tasks(
    ...
    # Also check file-based DLQ snapshots
    try:
...
async def get_dlq_snapshot(snapshot_id: str):
    ...
    try:
        ...
        if snapshot:
            # Strip base64 screenshots from response (use /telemetry/screenshot for those)
...
# ---------------------------------------------------------------------------
# Telemetry / Screenshot Viewer endpoints are imported via telemetry_router
# ---------------------------------------------------------------------------
...
async def resolve_workflow(resolution: HumanResolution):
    ...
    logger.info(
        f"[HITL] Operator '{resolution.operator_id}' issued "
        f"{resolution.decision} for workflow '{resolution.workflow_id}'"
    )

    # Publish resolution to the specific workflow's listener channel
    ...
    try:
        ...
        if published == 0:
            # No subscribers — try pushing to a fallback queue
            fallback_key = f"ufo:hitl:pending:{resolution.workflow_id}"
            r.setex(fallback_key, 900, payload)  # 15 min TTL
            logger.warning(
                f"[HITL] No active listener for '{resolution.workflow_id}'. "
                f"Resolution stored in fallback key (15 min TTL)."
            )
            return StatusResponse(
                status="pending",
                message="Agent not currently listening. Resolution stored for pickup.",
            )
        ...
        r.expire(audit_key, 86400 * 7)  # 7 day retention

        return StatusResponse(
            status="success",
            message=f"Resolution '{resolution.decision}' dispatched to agent.",
            data={"subscribers_notified": published},
        )
...
async def submit_workflow(submission: WorkflowSubmission):
    ...
    try:
        task = {
            ...
            "is_irrevocable": submission.is_irrevocable,
            "priority": submission.priority,
            "metadata": submission.metadata,
            "submitted_at": time.time(),
            "submitted_by": "control_plane",
        }
        ...
        return StatusResponse(
            status="success",
            message=f"Workflow '{task['workflow_id']}' submitted.",
