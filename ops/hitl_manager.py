...
class HITLManager:
    ...
    def wait_for_resolution(
        ...
        logger.critical(
            f"[HITL] Workflow '{workflow_id}' HALTED. "
            f"Awaiting human operator on channel '{channel}' "
            f"(timeout={timeout // 60} min)..."
        )

        if not self._available:
            # No Redis — check for local fallback
            return self._local_fallback(workflow_id, timeout)

        # Register that we're waiting (for Control Plane visibility)
        self._register_waiting(workflow_id, context)

        # Check for pre-queued resolution (if operator acted before we subscribed)
        ...
        try:
            ...
            while time.monotonic() < deadline:
                ...
                # get_message with timeout (non-blocking poll with sleep)
                ...
                if message and message["type"] == "message":
                    try:
                        ...
                        logger.info(
                            f"[HITL] Resolution received for '{workflow_id}': "
                ...
                # Also check fallback key periodically
            ...
            # Timeout
            logger.error(
                f"[HITL] TIMEOUT: No operator responded for workflow "
                f"'{workflow_id}' within {timeout // 60} minutes."
            )
            self._unregister_waiting(workflow_id)
            return {
                "decision": "ABORT",
    ...
    async def async_wait_for_resolution(
        self,
        workflow_id: str,
        timeout_minutes: Optional[int] = None,
        context: Optional[Dict[str, Any]] = None,
        ...
        logger.critical(
            f"[HITL] Workflow '{workflow_id}' HALTED (async). "
            f"Awaiting operator on '{channel}'..."
        )
        ...
        try:
            ...
            try:
                    ...
                    async for message in pubsub.listen():
                        if message["type"] == "message":
                            try:
                                resolution = json.loads(message["data"])
                                logger.info(
                                    f"[HITL] Resolution received (async): "
                                    f"{resolution.get('decision', 'UNKNOWN')}"
                                )
                                self._unregister_waiting(workflow_id)
                                return resolution
                ...
                logger.error(
                    f"[HITL] TIMEOUT (async): No response for '{workflow_id}' "
                    f"within {timeout // 60} minutes."
                )
                self._unregister_waiting(workflow_id)
                return {
                    "decision": "ABORT",
    ...
    def request_human_review(
        ...
        try:
            ...
            self._redis.setex(key, 3600, json.dumps(review_data))  # 1hr TTL

            # Also push to a review queue for the dashboard
            ...
            # Create DLQ snapshot if screenshot available
            if screenshot_path:
                try:
                    ...
                    dlq.capture_failure(
                        task_id=f"HITL_{workflow_id}",
                        error_chain=f"HITL escalation: {reason}",
                        dag_state=dag_state,
    ...
    def _register_waiting(
        ...
        try:
            ...
            self._redis.setex(
                f"ufo:hitl:waiting:{workflow_id}", 3600, json.dumps(info)
            )
        except Exception:
            pass
    ...
    def _local_fallback(
        self, workflow_id: str, timeout_seconds: float
    ) -> Dict[str, Any]:
        """
        ...
        """
        logger.warning(
            f"[HITL] No Redis — local fallback. "
            f"Workflow '{workflow_id}' will auto-ABORT after {timeout_seconds}s."
        )
        # Without Redis, we can't receive external signals.
        # Auto-ABORT after timeout.
        event = threading.Event()
        event.wait(timeout=timeout_seconds)
        return {
            "decision": "ABORT",
            "reason": "no_redis_hitl_timeout",
            "operator_id": "SYSTEM",
        }
