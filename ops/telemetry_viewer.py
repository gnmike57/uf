...
def decode_snapshot_screenshot(
...
async def serve_snapshot_screenshot(
    filename: str,
    key: str = Query(default="post", description="Screenshot key: pre, post, current"),
):
    ...
    if img_bytes is None:
        raise HTTPException(
            status_code=404,
            detail=f"Screenshot '{key}' not found in snapshot '{filename}'",
        )
...
async def get_snapshot_detail(filename: str):
    ...
    try:
        ...
        # Strip base64 data for API response (too large)
...
async def get_cost_history(
    ...
    try:
        ...
        for log_file in sorted(log_dir.glob("costs_*.json"), reverse=True)[:days]:
            try:
                with open(log_file, "r", encoding="utf-8") as f:
                    ...
                    history.append({
                        "date": log_file.stem.replace("costs_", ""),
                        "data": day_data,
                    })
            except Exception:
                continue
        ...
        return {
            "days": history,
            "total_usd": sum(
                d["data"].get("spent_today_usd", 0.0) for d in history
            ),
        }
...
def get_fleet_metrics() -> Dict[str, Any]:
    ...
    try:
        ...
        metrics["fleet"] = {
            ...
            "alive_workers": alive,
            "dead_workers": len(workers) - alive,
        }
        metrics["queues"] = {
            "global_pending": r.llen("ufo:queue:bankfidelity_tasks"),
            "dlq_depth": r.llen("ufo:queue:dlq"),
            "hitl_pending": r.llen("ufo:hitl:pending_reviews"),
        }
    except Exception:
