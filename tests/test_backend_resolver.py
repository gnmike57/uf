...
def create_minimal_profile(model_name: str) -> dict:
    return {
        "HOST_AGENT": {"API_TYPE": "openai", "API_MODEL": model_name},
        "APP_AGENT": {"API_TYPE": "openai", "API_MODEL": model_name}
    }
...
@pytest.mark.parametrize("state_content, is_raw", [
    (None, False),
    ("", True),
    ("hello", True),
    (["a"], False),
    ({"selected": "invalid"}, False),
    ({"selected": "profile"}, False), # missing path
])
...
def test_disk_fallback_ignores_other_profiles(temp_config_root):
    ...
    # Write agents_local_vision.yaml with a model that would shadow disk if merged
    write_yaml(
        temp_config_root / "ufo" / "agents_local_vision.yaml",
        create_minimal_profile("sneaky_local_override"),
    )
    write_yaml(
        temp_config_root / "ufo" / "agents_cloud.yaml",
        create_minimal_profile("sneaky_cloud_override"),
    )
    # No backend_state.json → disk fallback
    state_file = temp_config_root / "ufo" / "backend_state.json"
    ...
    cfg = get_agent_config(AgentType.HOST)
    assert cfg["API_MODEL"] == "disk_model", (
        "Disk fallback must load only agents.yaml, not merge other profile YAMLs"
    )
...
def test_auto_resolution(temp_config_root, monkeypatch):
    ...
    initial_probe_count = probe_count  # set_backend_selection probes once
    
    reset_backend_caches(clear_override=True)  # force fresh per-process memo
    assert get_agent_config(AgentType.HOST)["API_MODEL"] == "local_model"
    
    probe_after_first_read = probe_count
    # Exactly one additional probe after cache reset (memoized per-process)
    assert probe_after_first_read == initial_probe_count + 1
    
    # Even after many more reads, no additional probes
    for _ in range(5):
        get_agent_config(AgentType.HOST)
        get_backend_selection()
...
def test_auto_reevaluates_per_fresh_process_cache(temp_config_root, monkeypatch):
    ...
    # First: auto with local healthy
    monkeypatch.setattr(ch, "_probe_local_auto", lambda: True)
    set_backend_selection("auto")
    reset_backend_caches(clear_override=True)
    assert get_agent_config(AgentType.HOST)["API_MODEL"] == "local_model"

    # Simulate process restart: clear caches and change probe result
    reset_backend_caches(clear_override=True)
...
def test_env_var_expansion_in_profile(temp_config_root, monkeypatch):
    ...
    write_yaml(profile_path, {
        "HOST_AGENT": {"API_TYPE": "openai", "API_MODEL": "test_model", "API_KEY": "${TEST_UFO_API_KEY}"},
        "APP_AGENT": {"API_TYPE": "openai", "API_MODEL": "test_model"},
    })
    
    reset_backend_caches(clear_override=True)
    prof = resolve_backend_profile("profile", profile_path=str(profile_path))
    assert prof["HOST_AGENT"]["API_KEY"] == "sk-expanded-secret-key", (
        "Environment variable placeholder must be expanded in resolved profile"
    )
...
def test_no_write_invariant(temp_config_root):
    ...
    # Only backend_state.json should be new
...
def test_launcher_commands(temp_config_root, monkeypatch):
    ...
    # Load switch_backend
    sb_path = Path(r"C:\ufo\ufo\scripts\switch_backend.py")
    ...
    # Load prepare_cloud_smoke
    pcs_path = Path(r"C:\ufo\ufo\scripts\prepare_cloud_smoke.py")
    ...
    # New files: only backend_state.json is allowed
    ...
    # Explicitly check for dangerous residue patterns
    ...
    # Pre-existing files must be unchanged (except backend_state.json)
...
def test_real_directory_guard(temp_config_root, monkeypatch):
    real_config_dir = Path(r"C:\ufo\ufo\config\ufo")
    ...
    # No new files at all in the real config directory
    new_files = files_after - files_before
    assert not new_files, f"Launcher created new files in real config dir: {new_files}"

    # All existing files byte-identical
    assert hash_before == hash_after, (
        f"Launcher modified real config files: {set(hash_before.items()) ^ set(hash_after.items())}"
    )
