...
# Ensure project root is at the top of sys.path and remove tests dir from sys.path to avoid shadowing top-level galaxy package
...
# Suppress verbose ufo framework logs during high concurrency load tests
...
class IsolatedMockDeviceManager:
    ...
    def __init__(self, mock_latency: float = 0.001):
        ...
        self._connected_devices = {
            "web_device_01": {"device_type": "web", "status": "connected"},
            "office_device_01": {"device_type": "office", "status": "connected"},
            "mobile_device_01": {"device_type": "mobile", "status": "connected"},
            "desktop_device_01": {"device_type": "desktop", "status": "connected"},
            "cloud_service_01": {"device_type": "cloud", "status": "connected"},
        }
    ...
    async def assign_task_to_device(
        ...
        return ExecutionResult(
            task_id=task_id,
            status="completed",
            result={"message": f"Successfully executed '{task_description}' on {device_id}"},
            metadata={"device_id": device_id, "execution_time": self.mock_latency},
        )
...
class LoadMetricsCollector:
    ...
    def compute_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "framework_crashes": self.framework_crashes,
            ...
            "error_messages": errors_summary[:10],
        }
...
class UFOLoadTestRunner:
    ...
    def _create_synthetic_constellation(self, req_id: int, dag_type: str) -> TaskConstellation:
        ...
        if dag_type == "linear":
            tasks = [
                TaskStar(f"t_{req_id}_1", description="Init task", priority=TaskPriority.HIGH, target_device_id="desktop_device_01"),
                TaskStar(f"t_{req_id}_2", description="Process data", priority=TaskPriority.MEDIUM, target_device_id="office_device_01"),
                TaskStar(f"t_{req_id}_3", description="Validate output", priority=TaskPriority.HIGH, target_device_id="cloud_service_01"),
            ]
            for t in tasks:
        ...
        else: # complex branching
            tasks = [
                TaskStar(f"t_{req_id}_kickoff", description="Kickoff project", priority=TaskPriority.HIGH, target_device_id="desktop_device_01"),
                TaskStar(f"t_{req_id}_res", description="Research phase", priority=TaskPriority.MEDIUM, target_device_id="web_device_01"),
                TaskStar(f"t_{req_id}_arch", description="Design architecture", priority=TaskPriority.HIGH, target_device_id="office_device_01"),
                TaskStar(f"t_{req_id}_dev1", description="Develop module 1", priority=TaskPriority.MEDIUM, target_device_id="desktop_device_01"),
                TaskStar(f"t_{req_id}_dev2", description="Develop module 2", priority=TaskPriority.MEDIUM, target_device_id="desktop_device_01"),
                TaskStar(f"t_{req_id}_test", description="Integration test", priority=TaskPriority.HIGH, target_device_id="cloud_service_01"),
            ]
            for t in tasks:
                constellation.add_task(t)
    ...
    async def _execute_simplified_request(self, req_id: int, dag_type: str) -> Tuple[bool, int, int, Optional[str]]:
        ...
        try:
            ...
            # Execute topological task graph
    ...
    async def run(self) -> Dict[str, Any]:
        ...
        if self.profile == "ramp":
            # Ramp-up: stagger request starts evenly over ramp duration
            tasks = []
        ...
        # Display real-time telemetry summary
