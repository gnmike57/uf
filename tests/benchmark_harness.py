...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run UFO E2E benchmark timing harness."
    )
    parser.add_argument(
        "--task-id",
        "-t",
        default="baseline_notepad_001",
        help="Task ID for UFO run (default: baseline_notepad_001)",
    )
    parser.add_argument(
        "--mode",
        "-m",
        default="normal",
        help="UFO mode (default: normal)",
    )
    parser.add_argument(
        "--request",
        "-r",
        default="open Notepad and type 'hello world'",
        help="User request for UFO (default: 'open Notepad and type \\'hello world\\'')",
    )
    parser.add_argument(
        "--log-level",
        default="INFO",
        help="Logging level for UFO (default: INFO)",
    )
    parser.add_argument(
        "--python-exe",
        default=None,
        help="Python executable path to run UFO module (default: auto-detected)",
    )
    parser.add_argument(
        "--output-json",
        "-o",
        default=None,
        help="File path to save JSON benchmark report",
    )
...
def run_benchmark(task_id, mode, request, log_level, python_exe, output_json_path):
    ...
    # Collect step details or summary from logs if available
    step_details = []
    ...
    report = {
        ...
        "command": " ".join(ufo_cmd),
        "start_timestamp": start_timestamp,
        "end_timestamp": end_timestamp,
        "wall_clock_seconds": wall_clock_seconds,
        "returncode": returncode,
        "success": returncode == 0,
        "log_dir": str(log_dir.resolve()) if log_dir.exists() else None,
        "log_files": log_files,
