...
# Evaluation Stages Registry
EVAL_STAGES: Dict[str, Dict[str, Any]] = {
    "R1": stage_r1.get_stage_config(),
    "R2": stage_r2.get_stage_config(),
    "R3": stage_r3.get_stage_config(),
    "R4": stage_r4.get_stage_config(),
    "R5": stage_r5.get_stage_config(),
}
...
class EvaluationRunner:
    ...
    async def run_stage(
        ...
        request_text: str = str(
            request_override or stage_meta.get("default_request") or stage_meta.get("request") or ""
        )
        ...
        # Run pre-cleanup for stage if configured (only in live mode, offloaded to worker thread)
        if not self.dry_run:
        ...
        if self.dry_run:
            ...
            return {
                ...
                "target_app": stage_meta["target_app"],
                "task_name": task_name,
                ...
                "error": None,
                "trajectories": [{"step": 1, "action": "Dry run simulation completed successfully"}],
                "verification": verification_result,
                "log_dir": str(self.output_dir / task_name),
            }
        ...
        try:
            ...
            # Collect log trajectories from task output directory
    ...
    async def run_suite(
        ...
        if not stages or "ALL" in [s.strip().upper() for s in stages if s.strip()]:
            ...
            for s in stages:
                ...
                if su not in EVAL_STAGES:
                    ...
                    if su not in selected_stages:
                        selected_stages.append(su)
            if invalid_stages:
                raise ValueError(
                    f"Invalid evaluation stage(s): {invalid_stages}. Available stages: {list(EVAL_STAGES.keys()) + ['ALL']}"
                )
        ...
        if request_override and len(selected_stages) != 1:
            raise ValueError(
                f"Request override '--request' can only be specified when running a single stage (got {len(selected_stages)} stages: {selected_stages})."
            )
        ...
        for stage_id in selected_stages:
            ...
            res = await self.run_stage(
                stage_id=stage_id,
                request_override=req,
                task_name_override=t_name,
                mode=mode,
            )
        ...
        summary = {
            "title": "UFO 5-Stage Evaluation Suite Execution Report",
            "timestamp": timestamp,
            ...
            "execution_method": self.exec_method,
            "dry_run": self.dry_run,
            "stage_results": suite_results,
        }

        # Save structured JSON report and Markdown summary report (offloaded to thread to avoid blocking event loop)
        json_report_path, md_report_path = await asyncio.to_thread(
            _generate_unique_report_paths, self.output_dir, timestamp
        )
    ...
    def _write_markdown_summary(self, summary: Dict[str, Any], filepath: Path) -> None:
        ...
        lines = [
            ...
            f"- **Execution Method**: `{summary['execution_method']}` (Dry Run: `{summary['dry_run']}`)",
            f"- **Total Duration**: {summary['duration_seconds']}s",
            f"- **Results**: {summary['passed_stages']} Passed / {summary['failed_stages']} Failed (Total: {summary['total_stages']})",
            "",
            "## Stage Summary Table",
            "",
            "| Stage | Name | Target App | Status | Duration (s) | Task Name |",
            "|---|---|---|---|---|---|",
        ]

        for r in summary["stage_results"]:
            lines.append(
                f"| {r['stage_id']} | {r['stage_name']} | {r['target_app']} | **{r['status']}** | {r['duration_seconds']} | `{r['task_name']}` |"
            )
        ...
        for r in summary["stage_results"]:
            lines.extend([
                f"### Stage {r['stage_id']}: {r['stage_name']}",
                f"- **Request**: `{r['request']}`",
                f"- **Status**: `{r['status']}`",
                f"- **Start Time**: {r['start_time']}",
                f"- **End Time**: {r['end_time']}",
                f"- **Log Dir**: `{r['log_dir']}`",
            ])
            if r.get("error"):
...
def parse_args(args_list: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--stage",
        "-s",
        help="Evaluation stage(s) to run (R1, R2, R3, R4, R5, or ALL). Multiple can be comma-separated (e.g. R1,R2). Default: ALL",
        type=str,
        default="ALL",
    )
    parser.add_argument(
        "--request",
        "-r",
        help="Optional request description override for single stage execution.",
        type=str,
        default=None,
    )
    parser.add_argument(
        "--task",
        "-t",
        help="Optional custom task name or prefix.",
        type=str,
        default=None,
    )
    parser.add_argument(
        "--mode",
        "-m",
        help="Session mode (normal, follower, batch_normal, operator). Default: normal",
        type=str,
        default="normal",
    )
    parser.add_argument(
        "--exec-method",
        choices=["api", "cli"],
        help="Execution method: 'api' (SessionFactory) or 'cli' (python -m ufo). Default: api",
        default="api",
    )
    parser.add_argument(
        "--output-dir",
        "-o",
        help="Directory to save evaluation reports and structured results.",
        type=str,
        default=None,
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Run harness in dry-run simulation mode for test verification.",
    )
    parser.add_argument(
        "--log-level",
