...
def verify_r1(
    ...
    expected_message: str = DEFAULT_MESSAGE,
    target_filename: str = DEFAULT_FILENAME,
    dry_run: bool = False,
    ...
    if dry_run:
        return {
            ...
            "details": "[DRY RUN] Stage R1 Notepad Test verification simulated successfully.",
        }
    ...
    return {
        ...
        "stage_id": STAGE_ID,
        "dry_run": False,
        "file_exists": desktop_ver["exists"],
        "content_matched": desktop_ver["content_matched"],
        "file_path": desktop_ver["file_path"],
        "actual_content": desktop_ver.get("actual_content"),
        "expected_content": expected_message,
        "trajectory_verified": trajectory_ver.get("verified", False),
        "details": f"File exists: {desktop_ver['exists']}, Content match: {desktop_ver['content_matched']}, Path: {desktop_ver['file_path']}",
    }
...
def get_stage_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "description": "Execute a request via UFO to open Notepad, type a predefined message, and save the file to the Desktop.",
