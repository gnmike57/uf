...
def verify_file_on_desktop(
    filename: str = "ufo_test.txt",
    ...
    if not target_path.exists():
        # Fallback check in user home directory if not found on Desktop directly
        ...
        if home_path.exists():
            ...
            return {
                ...
                "actual_content": None,
                "error": f"File '{filename}' not found at '{target_path}'.",
            }
    ...
    try:
        ...
        return {
            ...
            "file_path": str(target_path),
            "actual_content": None,
            "error": f"Error reading file '{target_path}': {e}",
        }
    ...
    if not read_success:
        return {
            ...
            "file_path": str(target_path),
            "actual_content": None,
            "error": f"Failed to decode content of file '{target_path}'.",
        }
    ...
    return {
        "verified": content_matched,
        "exists": True,
        "content_matched": content_matched,
        "file_path": str(target_path),
        "actual_content": actual_content.lstrip("\ufeff").strip(),
        "error": None if content_matched else f"Content mismatch. Expected '{expected_content}', got '{actual_content.strip()}'",
    }
...
def verify_process_running(process_name: Union[str, List[str]]) -> Dict[str, Any]:
    ...
    if not psutil_success:
        # Fallback to tasklist CLI on Windows if psutil fails or is not available
        try:
    ...
    return {
        "verified": verified,
        "running_processes": list(set(running)),
        "error": None if verified else f"None of processes {process_names} were detected running.",
    }
...
def resolve_log_path(
    ...
    if not target and stage_data is not None:
        if isinstance(stage_data, (str, Path)):
            ...
            target = (
                stage_data.get("task_log_dir")
                or stage_data.get("log_dir")
                or stage_data.get("output_dir")
            )
...
def verify_session_logs(
    ...
    required_patterns: Optional[Union[str, List[str]]] = None,
    ...
    if log_dir is None:
        return {
            ...
            "matched_patterns": [],
            "errors": [],
            "error": "Log directory is None or not provided.",
        }
    ...
    if not log_path.is_dir():
        return {
            ...
            "matched_patterns": [],
            "errors": [],
            "error": f"Log path is not a directory: {log_path}",
        }
    ...
    if response_log.exists():
        ...
        if sub_logs:
            ...
            # Fallback to json step logs with deduplication via canonical path resolution
            seen_paths = set()
            json_logs = []
    ...
    if not log_files:
        return {
            ...
            "matched_patterns": [],
            "errors": [],
            "error": f"No log files found in '{log_path}'.",
        }
    ...
    for log_file in log_files:
        try:
            with open(log_file, "r", encoding="utf-8", errors="replace") as f:
                content = f.read()
                # Check JSON lines or single JSON file
...
def verify_bankfidelity_process(
    ...
    """
    targets = process_names or [
        "BankFidelity_Stable.exe",
        "dual-core-pdf-pipeline.exe",
        "bankfidelity.exe",
        "bankfidelity",
        "dual-core-pdf-pipeline",
    ]
