...
def test_tasks_and_dependencies_as_lists():
    ...
    task_list = [
        ...
        {
            "name": "Third Task",
            "description": "Task without preset ID",
        },
    ]
    ...
    dependency_list = [
        {
            ...
            "to_task_id": "task_002",
            "condition_description": "First dependency",
        },
        {
            "from_task_id": "task_002",
            "to_task_id": "task_003",
            "condition_description": "Second dependency",
        },
    ]

    constellation_data = {
        "name": "List format test constellation",
        "tasks": task_list,
    ...
    ), "Dependencies should be converted to Dict format"
    ...
    auto_generated_tasks = [
        task for task in constellation.tasks.values() if task.name == "Third Task"
    ]
...
def test_tasks_and_dependencies_as_dicts():
    ...
    task_dict = {
        "task_001": TaskStarSchema(
            task_id="task_001",
            name="Dict task 1",
            description="First dict task",
        ),
        "task_002": TaskStarSchema(
            task_id="task_002",
            name="Dict task 2",
            description="Second dict task",
        ),
    }
    ...
    dependency_dict = {
        "dep_001": TaskStarLineSchema(
            ...
            from_task_id="task_001",
            to_task_id="task_002",
            condition_description="Dict dependency",
        )
    }

    constellation = TaskConstellationSchema(
        name="Dict format test constellation", tasks=task_dict, dependencies=dependency_dict
    )
    ...
    assert isinstance(
        constellation.dependencies, dict
    ), "Dependencies should remain Dict format"
...
def test_mixed_format_compatibility():
    ...
    constellation1 = TaskConstellationSchema(
        name="Mixed constellation 1",
        tasks=[
            {"name": "List task 1", "description": "From list"},
            {"name": "List task 2", "description": "From list"},
        ],
        dependencies={
    ...
    print(
        f"[PASS] Mixed format 1 created: tasks={type(constellation1.tasks).__name__}, dependencies={type(constellation1.dependencies).__name__}"
    )

    constellation2 = TaskConstellationSchema(
        name="Mixed constellation 2",
        tasks={
            "manual_task": TaskStarSchema(
                task_id="manual_task", name="Dict task", description="From dict"
            )
        },
        dependencies=[
            {
                ...
                "condition_description": "From list dependency",
            }
        ],
    )

    print(
        f"[PASS] Mixed format 2 created: tasks={type(constellation2.tasks).__name__}, dependencies={type(constellation2.dependencies).__name__}"
    )
...
def test_conversion_methods():
    ...
    constellation = TaskConstellationSchema(
        name="Conversion test constellation",
        tasks=[
    ...
    print(
        f"[PASS] Exported as list format: tasks={type(data_with_lists['tasks']).__name__}, dependencies={type(data_with_lists['dependencies']).__name__}"
    )
...
def test_json_serialization():
    ...
    constellation = TaskConstellationSchema(
        name="JSON test constellation",
        tasks=[
            {"name": "JSON task 1", "description": "JSON desc 1"},
            {"name": "JSON task 2", "description": "JSON desc 2"},
        ],
        dependencies=[
            {
                "from_task_id": "task_001",
                "to_task_id": "task_002",
                "condition_description": "JSON dependency",
            }
        ],
    )
