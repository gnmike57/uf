...
def test_observer_modules():
    ...
    for observer_class, expected_module in expected_modules.items():
        ...
        assert actual_module == expected_module or actual_module.endswith(expected_module), (
            f"[FAIL] {observer_class.__name__} in wrong module: {actual_module} (expected: {expected_module})"
        )
