...
class ExecutionResult:
    ...
    __slots__ = (
        "status", "error", "coordinates", "plugin_used",
        "vision_stage", "settlement_passed", "redacted",
    )
    ...
    def __init__(self) -> None:
        ...
        self.vision_stage: Optional[str] = None  # "omniparser", "cloud_vlm", None
...
class AppAgentExecutor:
    ...
    def execute_node(
        ...
        logger.info(
            f"[Executor] Starting node '{node_id}': "
            f"action={action_type}, target={target_app}"
        )

        # ===================================================================
        # STEP 1: Security Gate — Sandbox + Auditor
        # ===================================================================
        try:
        ...
        # ===================================================================
        # STEP 2: Secure Credential Injection (if secure_type)
        # ===================================================================
        ...
        # ===================================================================
        # STEP 3: Plugin API Check (bypass GUI if available)
        # ===================================================================
        ...
        if plugin_result is not None:
            ...
            if not plugin_result:
                ...
                # Fall through to GUI execution
            else:
                return result

        # ===================================================================
        # STEP 4: Resolve Target Coordinates (UIA → Vision Fallback)
        # ===================================================================
        coordinates = self._step_resolve_coordinates(
            node, action_type, uia_tree, screenshot_path, application_window, result
        )
        ...
        # ===================================================================
        try:
            success = self._step_physical_execution(
                action_type, payload, coordinates, target_app
            )
            if not success:
                result.status = NodeStatus.FAILED.value
        ...
        logger.info(
            f"[Executor] Node '{node_id}' completed successfully. "
            f"Settlement: {'PASS' if result.settlement_passed else 'SKIP'}"
        )
        return result
    ...
    def _step_plugin_check(
        self, target_app: str, action_type: str, payload: str
    ) -> Optional[bool]:
        """
        ...
        result = self._plugin_mgr.try_execute(
        ...
        if result.success:
            logger.info(
                f"[Executor] Plugin '{result.plugin_used}' succeeded — GUI bypassed."
            )
            return True
        elif result.fell_back_to_gui:
            logger.info(
                f"[Executor] Plugin failed ({result.error}) — falling through to GUI."
            )
            return None  # Fall through
        else:
            return False

    def _step_resolve_coordinates(
        self,
        node: Any,
        action_type: str,
        uia_tree: Optional[Dict[str, Any]],
        screenshot_path: Optional[str],
        ...
        if self._vision:
            ...
            if bbox and bbox.center_x > 0 and bbox.center_y > 0:
                ...
                logger.info(
                    f"[Executor] Vision resolved: ({bbox.center_x}, {bbox.center_y}) "
                    f"via {bbox.source}, confidence={bbox.confidence:.2f}"
                )
                return coords
    ...
    def _step_physical_execution(
        ...
        try:
            if action_type == "click":
                ...
                # Window switching — handled by HostAgent, not AppAgent
    ...
    def _step_settlement(self, application_window: Any = None) -> bool:
        ...
        if not self._verifier:
            return True  # No verifier — assume settled

        try:
            settled = self._verifier.wait_for_settlement(
                application_window=application_window
            )
            return settled
        except Exception as e:
            logger.debug(f"[Executor] Settlement check failed: {e}")
            return True  # Don't fail the node over settlement
    ...
    @staticmethod
    def _resolve_from_uia(
        target_control: Dict[str, Any],
        uia_tree: Dict[str, Any],
    ) -> Optional[Tuple[int, int]]:
        """
        ...
        def _search(node: Dict[str, Any]) -> Optional[Tuple[int, int]]:
            ...
            if target_aid and node.get("automation_id") == target_aid:
                ...
                target_type
                and node.get("control_type") == target_type
                and target_name
                and target_name.lower() in node.get("name", "").lower()
            ):
                match = True
