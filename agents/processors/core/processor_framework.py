            ...
            for phase in ProcessingPhase:
                if phase in self.strategies:
                    ...
                    if not result.success:
                        ...
                        # CRITICAL: Set status to ERROR so the agent state machine
                        # transitions to ErrorState instead of NoneState (which
                        # silently ends the round with is_round_end=True).
                        self.processing_context.update_local({"status": "ERROR"})
                        self.logger.error(
                            f"Strategy {strategy.name} failed at phase {phase.value}: {result.error}. "
                            f"Status forced to ERROR to prevent silent round termination."
                        )
