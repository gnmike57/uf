...
class AppScreenshotCaptureStrategy(BaseProcessingStrategy):
    ...
    async def execute(
        ...
        try:
            ...
            # Step 5: Redact screenshot and UIA tree if downstream agent is cloud
            try:
                from ufo.llm import AgentType
                ...
                if is_cloud_agent_config(app_agent_cfg):
                    ...
                    if redactor.should_redact_for_model(is_cloud=True):
                        # Ensure UI tree was captured if possible
                        ...
                        redacted_screenshot_path, redacted_tree = redactor.redact_for_cloud(
                            clean_screenshot_path, raw_tree
                        )

                        if (
                            redacted_screenshot_path
                            and os.path.exists(redacted_screenshot_path)
                            and redacted_screenshot_path != clean_screenshot_path
                        ):
                            clean_screenshot_path = redacted_screenshot_path
                            from ufo.automator.ui_control.screenshot import PhotographerFacade

                            clean_screenshot_url = PhotographerFacade().encode_image_from_path(
                                redacted_screenshot_path
                            )
                ...
                def _normalize_action_dict(act_dict: dict):
                    ...
                    if isinstance(fn_val, str) and "(" in fn_val:
                        try:
                            ...
                            if isinstance(tree.body, ast.Call):
                                ...
                                for kw in tree.body.keywords:
                                    try:
                                        ...
                                        self.logger.debug(
                                            f"Failed to literal_eval AST keyword argument {kw.arg}: {lit_err}"
                                        )
                            ...
                            self.logger.debug(
                                f"Failed to parse function call AST expression: {parse_err}"
                            )
...
class AppActionExecutionStrategy(BaseProcessingStrategy):
    ...
    async def execute(
        ...
        try:
                        ...
                        if clean_screenshot_path and os.path.exists(clean_screenshot_path):
                            ...
                            if diff_percentage < 5.0:
                                ...
                                success_result = Result(
                                    ...
                                    return_data=fallback_result_msg,
                                    namespace="action"
                                )
                                
                                actions_list = self._create_action_info(
                                    annotation_dict,
                                    action,
                                    [success_result],
                                )
                                ...
                                return ProcessingResult(
                                    ...
                                    data={
                                        ...
                                        "action_info": action_info,
                                        "selected_control_screenshot_path": context.get_local("clean_screenshot_path", ""),
                                        "control_log": action_info.get_target_info() if action_info else [],
                                        "status": "CONTINUE",
                                    },
                                    phase=ProcessingPhase.ACTION_EXECUTION,
                                )
            ...
            # Recover gracefully instead of crashing the pipeline
            from ufo.aip.messages import Result, ResultStatus
            failed_result = Result(
                status=ResultStatus.FAILED,
                return_data=f"Action execution failed: {str(e)}. The UI state might have changed or a popup might be blocking the view. Please observe the new screenshot carefully and formulate a recovery plan.",
                namespace="action"
            )
            ...
            try:
                actions = self._create_action_info(
                    context.get_local("annotation_dict", {}),
                    context.get_local("parsed_response").action if context.get_local("parsed_response") else [],
                    [failed_result],
                )
                from ufo.agents.processors.schemas.actions import ListActionCommandInfo
                action_info = ListActionCommandInfo(actions)
...
class AppMemoryUpdateStrategy(BaseProcessingStrategy):
    ...
    async def execute(
        ...
        try:
            ...
            if is_stagnant:
                ...
                # If memory was cleared, the latest parsed_response should probably still be 
                # added so we have something, but wait, the prompt says "clear short-term memory".
                # Actually, adding the memory item *after* clear() is good so the agent remembers 
    ...
    def _execute_shake_up(self, agent: "AppAgent", context: ProcessingContext):
        ...
        try:
            ...
            warning_msg = (
                "WARNING: SHAKE-UP PROTOCOL INITIATED.\n"
                "You have been stuck in an infinite loop, attempting the exact same action 3 times in a row "
                "with no change to the UI. Your short-term memory has been wiped. "
                "The system just pressed the ESCAPE key for you. "
                "Please carefully examine the current screenshot and try a completely different approach!"
            )
