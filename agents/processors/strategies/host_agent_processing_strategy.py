...
class HostLLMInteractionStrategy(BaseProcessingStrategy):
    ...
    async def execute(
        ...
        try:
            ...
            # Step 1.5: Intercept BankFidelity static DAG execution (Constellation Logic)
            if "bankfidelity" in request.lower() or "bank statement" in request.lower():
                ...
                # We return a custom function call to build and execute constellation
                static_dag_config = {
                    "tasks": [
                        {"task_id": "open_bankfidelity", "name": "Open BankFidelity", "description": "Open BankFidelity App"},
                        {"task_id": "download_statement", "name": "Download Statement", "description": "Download Bank Statement from the app"}
                    ],
                    "dependencies": [
                        {"from_task_id": "open_bankfidelity", "to_task_id": "download_statement"}
                    ]
                }
                parsed_response = HostAgentResponse(
                    function="execute_constellation",
                    arguments={"config": static_dag_config, "clear_existing": True},
                    status="FINISH",
                    thought="Executing statically compiled DAG for BankFidelity to ensure 100% reliability.",
                    observation="Intercepted BankFidelity prompt."
                )
            ...
            if response_text == "Static DAG Interception":
                # We already created parsed_response above
                pass
            else:
    ...
    async def _get_llm_response_with_retry(
        ...
        for retry_count in range(max_retries):
            try:
                result = await host_agent.get_response(
                    prompt_message,
                    AgentType.HOST,
    ...
    def _validate_response_fields(self, response: HostAgentResponse) -> None:
        ...
        # Optionally allow PENDING if specifically requested
        import os
...
class HostActionExecutionStrategy(BaseProcessingStrategy):
    ...
    async def execute(
        ...
        try:
            ...
            if function_name == self.SELECT_APPLICATION_COMMAND:
                ...
                if function_name == "execute_constellation":
                    execution_result = await self._execute_constellation_dag(
    ...
    async def _execute_constellation_dag(
        self,
        parsed_response,
        ...
        try:
            ...
            # In a real environment, device_manager.initialize() might be needed, but we rely on local fallback
