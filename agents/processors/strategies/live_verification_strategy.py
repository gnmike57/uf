    ...
    async def execute(
        ...
        try:
            ...
            if command_dispatcher:
                # Ticket 3.4: Pixel-Stability Polling — wait until consecutive
                # screenshots have converged (stddev of pixel difference < threshold).
                # This correctly handles fade-in animations, loading spinners, etc.
                import hashlib
                prev_hash = None
                stable_count = 0
                max_polls = 10  # Max 5 seconds (10 polls × 0.5s)
                for _ in range(max_polls):
                    await asyncio.sleep(0.5)
                    try:
                        poll_results = await command_dispatcher.execute_commands(
                            [Command(tool_name="capture_window_screenshot",
                                     parameters={}, tool_type="data_collection")]
                        )
                        if poll_results and poll_results[0].status == ResultStatus.SUCCESS:
                            frame_hash = hashlib.md5(
                                str(poll_results[0].result)[:2048].encode()
                            ).hexdigest()
                            if frame_hash == prev_hash:
                                stable_count += 1
                                if stable_count >= 2:  # 2 consecutive identical frames = stable
                                    break
                            else:
                                stable_count = 0
                            prev_hash = frame_hash
                    except Exception:
                        break  # If screenshot fails, proceed with what we have
            ...
            if not result.verified:
                ...
                # Ticket 3.2: Automated Rollback — attempt Ctrl+Z undo for the failed action
                try:
                    import pyautogui
                    pyautogui.hotkey('ctrl', 'z')
                    self.logger.info("Automated rollback: Ctrl+Z undo sent to application")
                    
                    # If the verifier suggested a recovery action, log it for DAG re-routing
                    if result.suggested_recovery_action:
                        self.logger.info(
                            f"Suggested recovery action: {result.suggested_recovery_action}"
                        )
                ...
                return ProcessingResult(
                    success=False,
                    error=f"Visual Verification Failed: {result.failure_reason}",
                    data={
