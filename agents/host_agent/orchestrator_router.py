...
def _load_execution_config() -> Dict[str, Any]:
    ...
    defaults = {
        ...
        "ENABLE_SET_OF_MARKS": False,
        "ENABLE_API_PLUGINS": True,
    }
    try:
...
class OrchestratorRouter:
    ...
    def resolve_mode(
        self,
        user_intent: str = "",
        is_financial_routing: bool = False,
        ...
        """
        # Rule 1: Financial or irrevocable → ALWAYS DAG
        if is_financial_routing or is_irrevocable:
            logger.info(
                f"[Router] Forced DAG mode: "
                f"financial={is_financial_routing}, irrevocable={is_irrevocable}"
            )
            return ExecutionMode.DAG

        # Rule 2: Strict determinism → DAG
        if self.is_strict:
            logger.info("[Router] Strict determinism enabled → DAG mode.")
            return ExecutionMode.DAG

        # Rule 3: Exploratory mode
    ...
    def get_active_features(self) -> Dict[str, bool]:
        ...
        if self.is_strict:
            return {
                ...
                "set_of_marks": False,
                "api_plugins": self._config.get("ENABLE_API_PLUGINS", True),
                "human_in_the_loop": self._config.get("HUMAN_IN_THE_LOOP", False),
            }

        return {
            "experience_memory": self._config.get("ENABLE_EXPERIENCE_MEMORY", False),
            "set_of_marks": self._config.get("ENABLE_SET_OF_MARKS", False),
            "api_plugins": self._config.get("ENABLE_API_PLUGINS", True),
            "human_in_the_loop": self._config.get("HUMAN_IN_THE_LOOP", False),
        }
    ...
    def _execute_dag(
        self, user_intent: str, dag_engine: Any = None
    ) -> Dict[str, Any]:
        ...
        # The DAG engine integration point — callers provide their engine instance
        # This router doesn't own the engine, it just selects the mode
        return {
            "mode": ExecutionMode.DAG,
            "status": "routed",
            "intent": user_intent,
    ...
    def _execute_react(
        self, user_intent: str, react_engine: Any = None
    ) -> Dict[str, Any]:
        ...
        if features["experience_memory"]:
            try:
                ...
                if memory.is_available():
                    ...
                    if prior_knowledge:
                        logger.info(
                            "[ReAct] Experience memory matched — "
    ...
    def handle_fatal_failure(
        ...
        if self.human_in_the_loop:
            ...
            return self._serialize_to_dlq(
                task_id, error_trace, dag_state, screenshot_path
            )

    def _prompt_operator(self, task_id: str, error_trace: str) -> str:
        """Interactive CLI prompt for human-in-the-loop recovery."""
        logger.critical(
            f"[HITL] Fatal failure in task '{task_id}'. "
        ...
        try:
            response = input(
                "[UFO] Execution blocked. Provide guidance (or 'abort'): "
            ).strip()
    ...
    def _serialize_to_dlq(
        ...
        try:
            ...
            path = dlq.capture_failure(
                task_id=task_id,
                error_chain=error_trace,
                dag_state=dag_state,
