            ...
            if connected_devices_count < total_devices_count:
                ...
                if connected_count < total_devices_count:
                    self.display.print_warning(
                        f"??  Only {connected_count}/{total_devices_count} devices connected"
                    )
                    self.logger.warning(
                        f"??  Only {connected_count}/{total_devices_count} devices connected"
                    )
        ...
        try:
            ...
            if self._client:
                ...
                if connected_count < total_count:
                    self.display.print_warning(
                        f"??  Only {connected_count}/{total_count} devices connected for new session"
                    )
                    self.logger.warning(
                        f"??  Only {connected_count}/{total_count} devices connected"
                    )
                else:
                    self.display.print_success(
            ...
            if force and self._current_request_task:
                ...
                if task and not task.done():
                    ...
                    try:
                        ...
                        self.logger.warning(
                            "?? Task cancellation timed out, proceeding anyway"
                        )
                    except Exception as e:
