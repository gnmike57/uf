...
def _load_prices() -> Dict[str, Dict[str, float]]:
    ...
    # Fallback: try loading prices.yaml directly (two levels up: ufo package root)
    try:
        import yaml
...
# ---------------------------------------------------------------------------
# Cost Tracker (Singleton, Thread-Safe)
# ---------------------------------------------------------------------------
...
class CostTracker:
    ...
    def _load_config(self) -> None:
        ...
        try:
            ...
            if t_cfg and isinstance(t_cfg, dict):
                ...
                self._state.daily_budget_usd = float(
                    t_cfg.get("DAILY_BUDGET_USD", 50.0)
                )
    ...
    def record_usage(
        ...
        with self._state_lock:
            ...
            record = CallRecord(
                ...
                cost_usd=cost,
                cumulative_cost_usd=self._state.spent_today_usd,
            )
            self._call_history.append(record)

            if self._log_every_call:
                logger.debug(
                    f"[Telemetry] {api_type}/{model}: "
                    f"${cost:.4f} (today: ${self._state.spent_today_usd:.2f}"
                    f"/${self._state.daily_budget_usd:.2f})"
                )
            ...
            if pct >= self._warning_pct and not self._state.warning_issued:
                ...
                logger.warning(
                    f"[Telemetry] Budget warning: {pct:.0f}% of daily budget used "
                    f"(${self._state.spent_today_usd:.2f}/${self._state.daily_budget_usd:.2f})"
                )
            ...
            if self._state.spent_today_usd >= self._state.daily_budget_usd:
                if not self._state.budget_exceeded:
                    ...
                    logger.critical(
                        f"[Telemetry] DAILY BUDGET EXCEEDED: "
                        f"${self._state.spent_today_usd:.2f} >= "
                        f"${self._state.daily_budget_usd:.2f}. "
                        f"Cloud APIs LOCKED. Local models only."
                    )
    ...
    def _compute_cost(
        ...
        # Build the pricing key (matches prices.yaml format)
        ...
        if key in self._prices:
            ...
            return (
                (prompt_tokens / 1000.0) * rates.get("input", 0.0)
                + (completion_tokens / 1000.0) * rates.get("output", 0.0)
            )
        ...
        # Unknown model — log and return 0 (don't charge for unknown models)
    ...
    def _ensure_daily_reset(self) -> None:
        ...
        if self._state.date != today:
            ...
            logger.info(
                f"[Telemetry] Daily reset. Budget: ${self._state.daily_budget_usd:.2f}"
            )
    ...
    def _persist_daily_log(self) -> None:
        ...
        try:
            ...
            log_file = os.path.join(
                self._persist_path, f"cost_{self._state.date}.json"
            )

            log_data = {
                "date": self._state.date,
                "daily_budget_usd": self._state.daily_budget_usd,
                ...
                "calls": [r.model_dump() for r in self._call_history[-100:]],
            }
