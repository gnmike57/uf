...
class DeadLetterQueue:
    ...
    def __init__(self, output_dir: Optional[str]=None, snapshot_dir: Optional[str]=None, max_snapshots: int=100, enabled: bool=True, webhook_url: Optional[str]=None) -> None:
            ...
            if messages:
                for msg in reversed(messages):
                    if msg.get('role') == 'user':
                        ...
                        break
            snapshot_data: Dict[str, Any] = {'agent_type': str(agent_type), 'model': model, 'timestamp': time.time(), 'timestamp_iso': time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()), 'error': str(error), 'error_type': type(error).__name__, 'error_message': str(error), 'traceback': traceback.format_exc(), 'message_count': len(messages) if messages else 0, 'last_user_content_length': last_user_len, 'circuit_breaker_state': circuit_breaker_state}
            if extra_meta:
    ...
    def _dispatch_webhook(self, snapshot_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if webhook_url and str(webhook_url).strip():
                ...
                req = urllib.request.Request(str(webhook_url).strip(), data=json.dumps(snapshot_data).encode('utf-8'), headers={'Content-Type': 'application/json'}, method='POST')
...
def record_dlq_event(agent_type: str, messages: List[Dict[str, Any]], error: Exception, model: str='', circuit_breaker_state: str='UNKNOWN', extra_meta: Optional[Dict[str, Any]]=None) -> Optional[Path]:
    """
    Convenience function to record a DLQ snapshot using the configured default DLQ instance.
    """
    try:
        return get_default_dlq().record_failure(agent_type=agent_type, messages=messages, error=error, model=model, circuit_breaker_state=circuit_breaker_state, extra_meta=extra_meta)
    except Exception as e:
