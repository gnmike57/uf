...
from ufo.aip.messages import ControlInfo
...
class WindowsAppEnv:
    ...
    def start(self, copied_template_path: str) -> None:
        ...
        try:
            file_controller.execute_code({'APP': self.win_app, 'file_path': copied_template_path})
    ...
    def _match_window_name(self, window_title: str, doc_name: str) -> bool:
        ...
        if _MATCH_STRATEGY == 'contains':
            ...
            return re.search(pattern_1, window_title) is not None or re.search(pattern_2, window_title) is not None
        else:
    ...
    def find_matching_controller(self, filtered_annotation_dict: Dict[int, ControlInfo], control_text: str) -> Tuple[str, ControlInfo]:
        """ "
