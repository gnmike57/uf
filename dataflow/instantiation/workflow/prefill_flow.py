...
from ufo.dataflow.config.config import Config
from ufo.dataflow.instantiation.agent.prefill_agent import PrefillAgent
from ufo.dataflow.env.env_manager import WindowsAppEnv
...
class PrefillFlow(AppAgentProcessor):
    ...
    def __init__(self, app_name: str, task_file_name: str, environment: WindowsAppEnv) -> None:
        """
        ...
        if self._app_name not in PrefillFlow._app_prefill_agent_dict:
            PrefillFlow._app_prefill_agent_dict[self._app_name] = PrefillAgent('prefill', self._app_name, is_visual=True, main_prompt=_configs['PREFILL_PROMPT'], example_prompt=_configs['PREFILL_EXAMPLE_PROMPT'], api_prompt=_configs['API_PROMPT'])
        ...
        self._status = ''
        self._log_path_configs = _configs['PREFILL_LOG_PATH'].format(task=self._task_file_name)
        os.makedirs(self._log_path_configs, exist_ok=True)
        self._message_logger = BaseSession.initialize_logger(self._log_path_configs, 'prefill_messages.json', 'w', _configs)
        self._response_logger = BaseSession.initialize_logger(self._log_path_configs, 'prefill_responses.json', 'w', _configs)

    def execute(self, template_copied_path: str, original_task: str, refined_steps: List[str]) -> Dict[str, Any]:
        """
        ...
        try:
            instantiated_request, instantiated_plan = self._instantiate_task(template_copied_path, original_task, refined_steps)
        except Exception as e:
            raise e
        finally:
            self.execution_time = round(time.time() - start_time, 3)
        return {'instantiated_request': instantiated_request, 'instantiated_plan': instantiated_plan}

    def _instantiate_task(self, template_copied_path: str, original_task: str, refined_steps: List[str]) -> Tuple[str, List[str]]:
        """
        ...
        """
        try:
            instantiated_request, instantiated_plan = self._get_prefill_actions(original_task, refined_steps, template_copied_path)
    ...
    def _update_state(self, file_path: str) -> None:
        ...
        control_list = self._control_inspector.find_control_elements_in_descendants(self._app_env.app_window, control_type_list=_ufo_configs['CONTROL_LIST'], class_name_list=_ufo_configs['CONTROL_LIST'])
        self._annotation_dict = self._photographer.get_annotation_dict(self._app_env.app_window, control_list, annotation_type='number')
        self._filtered_annotation_dict = self.get_filtered_annotation_dict(self._annotation_dict, configs=_configs)
        self._control_info = self._control_inspector.get_control_info_list_of_dict(self._annotation_dict, ['control_text', 'control_type' if _BACKEND == 'uia' else 'control_class'])
        self._filtered_control_info = self._control_inspector.get_control_info_list_of_dict(self._filtered_annotation_dict, ['control_text', 'control_type' if _BACKEND == 'uia' else 'control_class'])

    def _get_prefill_actions(self, given_task: str, reference_steps: List[str], file_path: str) -> Tuple[str, List[str]]:
        """
        ...
        prompt_message = self._prefill_agent.message_constructor('', given_task, reference_steps, self._log_path_configs)
        self._log_message(prompt_message)
        try:
            start_time = time.time()
            import asyncio
            llm_result = asyncio.run(self._prefill_agent.get_response(prompt_message, 'prefill', use_backup_engine=True))
    ...
    def _log_message(self, prompt_message: str) -> None:
        ...
        """
        messages_log_entry = {'step': self._execute_step, 'messages': prompt_message, 'error': ''}
        self._message_logger.info(json.dumps(messages_log_entry, indent=4))

    def _log_response(self, response_json: Dict[str, Any], execution_time: float) -> None:
        """
        ...
        """
        response_log_entry = {'step': self._execute_step, 'execution_time': execution_time, 'agent_response': response_json, 'error': ''}
    ...
    def _save_screenshot(self, doc_name: str, save_path: str) -> None:
        ...
        try:
            ...
            if matched_window:
                screenshot = self._photographer.capture_app_window_screenshot(matched_window)
            else:
