...
from ufo.dataflow.instantiation.agent.filter_agent import FilterAgent
...
class FilterFlow:
    ...
    def _get_or_create_filter_agent(self) -> FilterAgent:
        ...
        if self._app_name not in FilterFlow._app_filter_agent_dict:
            FilterFlow._app_filter_agent_dict[self._app_name] = FilterAgent('filter', self._app_name, is_visual=True, main_prompt=_configs['FILTER_PROMPT'], example_prompt='', api_prompt=_configs['API_PROMPT'])
    ...
    def execute(self, instantiated_request: str) -> Dict[str, Any]:
        ...
        try:
            judge, thought, request_type = self._get_filtered_result(instantiated_request)
    ...
    def _initialize_logs(self) -> None:
        ...
        self._filter_message_logger = BaseSession.initialize_logger(self._log_path_configs, 'filter_messages.json', 'w', _configs)
        self._filter_response_logger = BaseSession.initialize_logger(self._log_path_configs, 'filter_responses.json', 'w', _configs)
    ...
    def _get_filtered_result(self, instantiated_request: str) -> Tuple[bool, str, str]:
        ...
        try:
            start_time = time.time()
            import asyncio
            llm_result = asyncio.run(self._filter_agent.get_response(prompt_message, 'filter', use_backup_engine=True))
            ...
            return (response_json['judge'], response_json['thought'], response_json['type'])
        except Exception as e:
