...
from ufo.dataflow.instantiation.agent.template_agent import TemplateAgent
...
class ChooseTemplateFlow:
    ...
    def __init__(self, app_name: str, task_file_name: str, file_extension: str):
        ...
        self._embedding_model = self._load_embedding_model(model_name=_configs['CONTROL_FILTER_MODEL_SEMANTIC_NAME'])
    ...
    def _get_chosen_file_path(self) -> str:
        ...
        """
        templates_description_path = Path(_configs['TEMPLATE_PATH']) / self._app_name / 'description.json'
        try:
            with open(templates_description_path, 'r', encoding='utf-8') as f:
    ...
    def _choose_template_and_copy(self) -> str:
        ...
        chosen_template_full_path = Path(_configs['TEMPLATE_PATH']) / self._app_name / chosen_template_file_path
        target_template_folder_path = Path(_configs['RESULT_HUB'].format(task_type='saved_document')) / os.path.dirname(os.path.dirname(self._task_file_name))
    ...
    def _choose_target_template_file(self, given_task: str, doc_files_description: Dict[str, str]) -> str:
        ...
        if _configs['TEMPLATE_METHOD'] == 'SemanticSimilarity':
            return self._choose_target_template_file_semantic(given_task, doc_files_description)
        elif _configs['TEMPLATE_METHOD'] == 'LLM':
            self.template_agent = TemplateAgent('template', is_visual=True, main_prompt=_configs['TEMPLATE_PROMPT'])
            return self._choose_target_template_file_llm(given_task, doc_files_description)
        else:
            raise ValueError('Invalid TEMPLATE_METHOD.')

    def _choose_target_template_file_semantic(self, given_task: str, doc_files_description: Dict[str, str]) -> str:
        """
        Get the target file based on the semantic similarity of the given task and the template file descriptions.
        :param given_task: The task to be matched.
        :param doc_files_description: A dictionary of template file descriptions.
    ...
    def _choose_target_template_file_llm(self, given_task: str, doc_files_description: Dict[str, str]) -> str:
        """
        ...
        :return: The path to the chosen template file.
        """
        prompt_message = self.template_agent.message_constructor(doc_files_description, given_task)
        import asyncio
        llm_result = asyncio.run(self.template_agent.get_response(prompt_message, 'prefill', use_backup_engine=True))
