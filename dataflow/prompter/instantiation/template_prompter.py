...
class TemplatePrompter(BasicPrompter):
    ...
    def file_prompt_helper(self, path) -> str:
        ...
        for i in range(len(image_urls)):
            user_content.append({'type': 'text', 'text': 'This is the screenshot of ' + str(i + 1) + '.docx'})
            user_content.append({'type': 'image_url', 'image_url': {'url': image_urls[i]}})
        return user_content
    ...
    def user_prompt_construction(self, request: str) -> str:
        ...
        """
        prompt = self.prompt_template['user'].format(given_task=sanitize_user_input(request, 'given_task'))
        return prompt
    ...
    def user_content_construction(self, path: str, request: str) -> List[Dict]:
        ...
        user_content.append({'type': 'text', 'text': self.user_prompt_construction(request)})
        return user_content