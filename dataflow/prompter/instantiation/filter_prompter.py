...
class FilterPrompter(BasicPrompter):
    ...
    def api_prompt_helper(self, apis: Dict=None, verbose: int=1) -> str:
        """
        ...
        if len(apis) == 0:
            ...
            for key in apis.keys():
                api = apis[key]
                api_text = '{description}\n{example}'.format(description=api['description'], example=api['example'])
                api_list.append(api_text)
            api_prompt = self.retrieved_documents_prompt_helper('', '', api_list)
        return api_prompt
    ...
    def user_prompt_construction(self, request: str) -> str:
        ...
        """
        prompt = self.prompt_template['user'].format(request=sanitize_user_input(request, 'request'))
        return prompt
