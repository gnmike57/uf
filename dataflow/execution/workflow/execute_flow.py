...
from ufo.dataflow.config.config import Config as InstantiationConfig
from ufo.dataflow.env.env_manager import WindowsAppEnv
from ufo.dataflow.execution.agent.execute_agent import ExecuteAgent
from ufo.dataflow.execution.agent.execute_eval_agent import ExecuteEvalAgent
...
class ExecuteFlow(AppAgentProcessor):
    ...
    def __init__(self, task_file_name: str, context: Context, environment: WindowsAppEnv) -> None:
        """
    ...
    def _get_or_create_execute_agent(self) -> ExecuteAgent:
        ...
        if self._app_name not in ExecuteFlow._app_execute_agent_dict:
            ExecuteFlow._app_execute_agent_dict[self._app_name] = ExecuteAgent('execute', self._app_name, self._app_env.app_root_name)
    ...
    def _get_or_create_evaluation_agent(self) -> ExecuteEvalAgent:
        ...
        if self._app_name not in ExecuteFlow._app_eval_agent_dict:
            ExecuteFlow._app_eval_agent_dict[self._app_name] = ExecuteEvalAgent('evaluation', self._app_env.app_root_name, is_visual=True, main_prompt=_ufo_configs['EVALUATION_PROMPT'], example_prompt='', api_prompt=_ufo_configs['API_PROMPT'])
    ...
    def _initialize_logs(self, log_path: str) -> None:
        ...
        self._execute_message_logger = BaseSession.initialize_logger(log_path, 'execute_log.json', 'w', _configs)
        self.context.set(ContextNames.LOG_PATH, log_path)
        self.context.set(ContextNames.LOGGER, self._execute_message_logger)

    def execute(self, request: str, instantiated_plan: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], Dict[str, str]]:
        """
        ...
        try:
            ...
            if loop and loop.is_running():
                with concurrent.futures.ThreadPoolExecutor(1) as pool:
                    result, _ = pool.submit(asyncio.run, self.eval_agent.evaluate(request=request, log_path=self.log_path)).result()
            else:
                result, _ = asyncio.run(self.eval_agent.evaluate(request=request, log_path=self.log_path))
    ...
    def execute_plan(self, instantiated_plan: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        try:
            time.sleep(1)
            self.app_agent.Puppeteer.receiver_manager.create_api_receiver(self.app_agent._app_root_name, self.app_agent._process_name)
    ...
    def print_step_info(self) -> None:
        ...
        """
        utils.print_with_color('Step {step}: {subtask}'.format(step=self.session_step, subtask=self.subtask), 'magenta')

    def log_save(self) -> None:
        """
        Log the constructed prompt message for the PrefillAgent.
        """
        step_memory = {'Step': self.session_step, 'Subtask': self.subtask, 'ControlLabel': self._control_label, 'ControlText': self.control_text, 'Action': self.action, 'ActionType': self.app_agent.Puppeteer.get_command_types(self._operation), 'Results': self._results, 'Application': self.app_agent._app_root_name, 'TimeCost': self.time_cost}
    ...
    def _parse_step_plan(self, step_plan: Dict[str, Any]) -> None:
        ...
        self.action = self.app_agent.Puppeteer.get_command_string(self._operation, self._args)
    ...
    def init_and_final_capture_screenshot(self) -> None:
        ...
        self._memory_data.add_values_from_dict({'CleanScreenshot': screenshot_save_path})
        self.photographer.capture_app_window_screenshot(self.application_window, save_path=screenshot_save_path)
    ...
    def execute_action(self) -> None:
        ...
        if self.control_text == '':
            ...
            self._control_label, control_selected = self._app_env.find_matching_controller(self.filtered_annotation_dict, self.control_text)
            if control_selected:
        ...
        try:
            if control_selected:
                ...
                control_coordinates = utils.coordinate_adjusted(self.application_window.rectangle(), control_selected.rectangle())
                self._control_log = {'control_class': control_selected.element_info.class_name, 'control_type': control_selected.element_info.control_type, 'control_automation_id': control_selected.element_info.automation_id, 'control_friendly_class_name': control_selected.friendly_class_name(), 'control_coordinates': {'left': control_coordinates[0], 'top': control_coordinates[1], 'right': control_coordinates[2], 'bottom': control_coordinates[3]}}
                self.app_agent.Puppeteer.receiver_manager.create_ui_control_receiver(control_selected, self.application_window)
                self.capture_control_screenshot(control_selected)
