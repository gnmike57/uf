...
from ufo.dataflow.instantiation.workflow.choose_template_flow import ChooseTemplateFlow
from ufo.dataflow.instantiation.workflow.prefill_flow import PrefillFlow
from ufo.dataflow.instantiation.workflow.filter_flow import FilterFlow
from ufo.dataflow.execution.workflow.execute_flow import ExecuteFlow
from ufo.dataflow.config.config import Config
from ufo.utils import print_with_color
from ufo.learner.utils import load_json_file, save_json_file, reformat_json_file
...
EXECUTION_RESULT_MAP = {'yes': 'execution_pass', 'no': 'execution_fail', 'unsure': 'execution_unsure'}
...
class DataFlowController:
    ...
    def execute_instantiation(self) -> Optional[List[Dict[str, Any]]]:
        ...
        template_copied_path = self.instantiation_single_flow(ChooseTemplateFlow, 'choose_template', init_params=[self.task_object.app_object.file_extension], execute_params=[])
        if template_copied_path:
            self.app_env.start(template_copied_path)
            prefill_result = self.instantiation_single_flow(PrefillFlow, 'prefill', init_params=[self.app_env], execute_params=[template_copied_path, self.task_object.task, self.task_object.refined_steps])
            self.app_env.close()
            if prefill_result:
                self.instantiation_single_flow(FilterFlow, 'instantiation_evaluation', init_params=[], execute_params=[prefill_result['instantiated_request']])
    ...
    def execute_execution(self, request: str, plan: Dict[str, any]) -> None:
        ...
        try:
            ...
            self.task_info['execution_result']['error'] = {'type': str(type(e).__name__), 'message': str(e), 'traceback': traceback.format_exc()}
    ...
    def instantiation_single_flow(self, flow_class: AppAgentProcessor, flow_type: str, init_params=None, execute_params=None) -> Optional[Dict[str, Any]]:
        """
    ...
    def save_result(self) -> None:
        ...
        elif not self.task_info['execution_result']['result']:
    ...
    def reformat_to_batch(self, path) -> None:
        ...
        source_files_path = os.path.join(self.result_hub, self.task_type + '_pass')
        source_template_path = os.path.join(os.path.dirname(self.result_hub), 'saved_document')
        target_file_path = os.path.join(path, 'tasks')
        target_template_path = os.path.join(path, 'files')
        ...
        for file in os.listdir(source_files_path):
            if file.endswith('.json'):
                ...
                target_object = os.path.join(target_template_path, file.replace('.json', '.docx'))
                is_successed = reformat_json_file(target_file, target_object, load_json_file(source_file))
                if is_successed:
                    shutil.copy(os.path.join(source_template_path, file.replace('.json', '.docx')), target_template_path)
