...
def _win32_print_window(hwnd: int) -> Optional[Image.Image]:
    ...
    try:
        ...
        # Guard Clause: Prevent invalid dimensions from crashing the API
        ...
        try:
            ...
            # RAII Pattern: Guarantee GDI handle cleanup even if Image.frombuffer throws an exception
            save_dc.DeleteDC()
            mfc_dc.DeleteDC()
            win32gui.ReleaseDC(hwnd, hwnd_dc)
            win32gui.DeleteObject(bmp.GetHandle())

    # Catch specific exceptions instead of swallowing critical errors with bare except
...
def _win32_grab_screen() -> Optional[Image.Image]:
    ...
    try:
        ...
        try:
            ...
            # RAII Pattern: Prevent GDI resource leak on validation failure
            mem_dc.DeleteDC()
            img_dc.DeleteDC()
    ...
    try:
        ...
        # Eradicate Magic Numbers with explicit constants
        ...
        if width == 0: width, height = DEFAULT_FALLBACK_WIDTH, DEFAULT_FALLBACK_HEIGHT
        ...
        def callback(hwnd, windows_list):
            if win32gui.IsWindowVisible(hwnd) and win32gui.GetWindowText(hwnd):
                rect = win32gui.GetWindowRect(hwnd)
                # Filter out minimized/invisible bounds via Guard Clause
