...
class CUASkills:
    ...
    def save_as(self, file_path: str) -> str:
        ...
        try:
            ...
            time.sleep(1.0)  # Wait for Save As dialog
            pyautogui.hotkey('ctrl', 'a')
            time.sleep(0.2)
            pyautogui.typewrite(file_path, interval=0.02) if file_path.isascii() else pyautogui.write(file_path)
            time.sleep(0.3)
            ...
            # Handle "file already exists" confirmation
            time.sleep(0.5)
    ...
    def switch_to_window(self, title_fragment: str) -> str:
        ...
        try:
            ...
            # Use EnumWindows to find the target window
    ...
    def wait_for_window(
        self, title_fragment: str, timeout: float = 30.0, poll_interval: float = 1.0
    ) -> bool:
        """
    ...
    def screenshot_region(
        self, x: int, y: int, width: int, height: int, save_path: str
    ) -> str:
        """
